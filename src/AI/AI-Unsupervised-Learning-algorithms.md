...
K-Means es un algoritmo de agrupamiento basado en centroides que partitiona los datos en K grupos asignando cada punto al centroide del grupo más cercano. El algoritmo funciona de la siguiente manera:
...
K-Means asume que **los grupos son esféricos y de tamaño igual**, lo cual puede no ser cierto para todos los conjuntos de datos. Es sensible a la colocación inicial de los centroides y puede converger a mínimos locales. Además, K-Means no es adecuado para conjuntos de datos con densidades variables o formas no globulares y características con diferentes escalas. Los pasos de preprocesamiento como la normalización o estandarización pueden ser necesarios para asegurar que todas las características contribuyan igualmente a los cálculos de distancia.
...
```
En este ejemplo, K-Means debería encontrar 4 clústeres. El clúster de ataque pequeño (con una duración inusualmente alta ~200) idealmente formará su propio clúster dado su distancia de los clústeres normales. Imprimimos los tamaños y centros de los clústeres para interpretar los resultados. En un escenario real, uno podría etiquetar el clúster con pocos puntos como posibles anomalías o inspeccionar sus miembros en busca de actividad maliciosa.
</details>
...
DBSCAN es un algoritmo de clustering basado en densidad que agrupa puntos que están estrechamente agrupados mientras marca puntos en regiones de baja densidad como atípicos. Es particularmente útil para conjuntos de datos con densidades variables y formas no esféricas.
...
```
donde:
- \( \pi_k \) es el coeficiente de mezcla para el clúster k (probabilidad previa del clúster k),
