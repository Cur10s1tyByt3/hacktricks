...
### 1. Pondération de Mélange Basée sur la Température
Les LLMs à la pointe de la technologie sont rarement entraînés sur un seul corpus. Au lieu de cela, ils échantillonnent à partir de plusieurs sources de données hétérogènes (code, web, articles académiques, forums…). La proportion relative de chaque source peut fortement affecter la performance en aval. Des modèles open-source récents tels que Llama 2 ont introduit un **schéma d'échantillonnage basé sur la température** où la probabilité de tirer un document du corpus *i* devient
```
p(i) = \frac{w_i^{\alpha}}{\sum_j w_j^{\alpha}}
```
• *w<sub>i</sub>*  – pourcentage brut de jetons du corpus *i*  
• *α* ("température") – une valeur dans (0,1].  α < 1 aplatit la distribution, donnant plus de poids à des corpus de haute qualité plus petits.

Llama 2 a utilisé α = 0.7 et a montré que diminuer α augmentait les scores d'évaluation sur des tâches riches en connaissances tout en maintenant le mélange d'entraînement stable.  Le même truc est adopté par Mistral (2023) et Claude 3.
```python
