...
Para maximizar la probabilidad del token correcto, se deben modificar los pesos del modelo para que esa probabilidad se maximice. Las actualizaciones de los pesos se realizan a través de **retropropagación**. Esto requiere una **función de pérdida a maximizar**. En este caso, la función será la **diferencia entre la predicción realizada y la deseada**.
...
Estas son algunas funciones simples que se pueden utilizar para transformar textos del vocabulario a ids y viceversa. Esto es necesario al principio del manejo del texto y al final de las predicciones:
```python
# Functions to transform from tokens to ids and from to ids to tokens
def text_to_token_ids(text, tokenizer):
encoded = tokenizer.encode(text, allowed_special={'<|endoftext|>'})
...
Ambos cargadores de datos utilizan el mismo tamaño de lote, longitud máxima, y stride y num workers (0 en este caso).\
...
- Se llama a `optimizer.zero_grad()` para restablecer los gradientes en cada ronda y no acumularlos.
