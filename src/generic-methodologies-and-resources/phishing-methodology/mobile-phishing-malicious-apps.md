...
> [!INFO]
> Cette page couvre les techniques utilisées par les acteurs de la menace pour distribuer des **APK Android malveillants** et des **profils de configuration mobile iOS** via le phishing (SEO, ingénierie sociale, fausses boutiques, applications de rencontre, etc.).
...
* Enregistrer des dizaines de domaines similaires (rencontre, partage de cloud, service automobile…).
...
* Le code est **POSTé sur HTTP** vers le Command-and-Control (C2).
...
* **Diff Manifest vs. Runtime** – Comparez `aapt dump permissions` avec `PackageManager#getRequestedPermissions()` à l'exécution ; l'absence de permissions dangereuses est un signal d'alerte.
