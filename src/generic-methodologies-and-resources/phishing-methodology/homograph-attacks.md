...
Un ataque de homografía (también conocido como homoglifos) abusa del hecho de que muchos **puntos de código Unicode de scripts no latinos son visualmente idénticos o extremadamente similares a caracteres ASCII**. Al reemplazar uno o más caracteres latinos con sus contrapartes visualmente similares, un atacante puede crear:

* Nombres de visualización, asuntos o cuerpos de mensajes que parecen legítimos a la vista humana pero evitan detecciones basadas en palabras clave.
...
2. **Registrar infraestructura de soporte** – Opcionalmente registrar un dominio homoglífico y obtener un certificado TLS (la mayoría de las CAs no realizan verificaciones de similitud visual).
3. **Enviar correo electrónico / SMS** – El mensaje contiene homoglifos en una o más de las siguientes ubicaciones:
* Nombre de visualización del remitente (por ejemplo, `Ηеlрdеѕk`)
* Línea de asunto (`Urgеnt Аctіon Rеquіrеd`)
* Texto del hipervínculo o nombre de dominio completamente calificado
4. **Cadena de redirección** – La víctima es redirigida a través de sitios web aparentemente benignos o acortadores de URL antes de aterrizar en el host malicioso que recopila credenciales / entrega malware.
...
3. Generar una alerta si hay más de un script presente **o** si aparecen scripts no latinos donde no se esperan (nombre de visualización, dominio, asunto, URL, etc.).
...
* Implementar la lógica de detección anterior en **Secure Email Gateways** y libros de jugadas de **SIEM/XSOAR**.
...
* Nombre de visualización: `Сonfidеntiаl Ꭲiꮯkеt` (Cirílico `С`, `е`, `а`; Cherokee `Ꭲ`; capital pequeño latino `ꮯ`).
