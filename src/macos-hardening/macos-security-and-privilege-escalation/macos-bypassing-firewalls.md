...
- Si le pare-feu demande la permission à l'utilisateur, faites en sorte que le malware **clique sur autoriser**
...
Une URI spécialement conçue (par exemple, avec un double “://” encodé en URL) n'est pas reconnue par l'ACL de Temps d'écran mais est acceptée par WebKit, donc la requête est envoyée sans filtre. Tout processus capable d'ouvrir une URL (y compris le code sandboxé ou non signé) peut donc atteindre des domaines qui sont explicitement bloqués par l'utilisateur ou un profil MDM.
