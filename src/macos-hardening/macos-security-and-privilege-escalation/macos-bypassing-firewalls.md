...
En julio de 2024, Apple corrigió un error crítico en Safari/WebKit que rompió el “filtro de contenido web” a nivel del sistema utilizado por los controles parentales de Tiempo de pantalla. 
...
```
### Abusando de servicios auxiliares firmados por Apple (heredado – pre-macOS 11.2)
Antes de macOS 11.2, la **`ContentFilterExclusionList`** permitía ~50 binarios de Apple como **`nsurlsessiond`** y la App Store eludir todos los firewalls de filtro de socket implementados con el marco de Extensión de Red (LuLu, Little Snitch, etc.).
...
```
2. Enumerar los binarios que ya tienen el derecho de *outgoing-network* (útil para piggy-backing):
```bash
